...
- [x] 2. [Evidence automation — `./.github/workflows/ci.yml`, `./.github/workflows/docs.yml`, `./src/test/kotlin/eu/inqudium/tabellarium/KafkaBrokerIntegrationTest.kt`, `./pom.xml:538`] {Medium} {Confidence: high} {Testability & Test Architecture} The only wire-level evidence (broker stage) is executed by no automation
  ...
  - **Status:** Fixed in `9ce93df` (2026-08-29) — new `broker-integration` CI job on every push, PR, and the weekly schedule, running `-Pintegration -Dgroups=integration` (only the broker tests; the offline suite stays with the build job) plus its own test summary. The job command was verified locally (exactly 1 test, green) before wiring.

- [x] 3. [Architecture self-description — `./README.md:687` (diagram), `./README.md:718` (component contract paragraph)] {Medium} {Confidence: high} {Consistency} The README architecture diagram still describes the pre-SendDispatcher system
  ...
  - **Status:** Fixed in `9ce93df` (2026-08-29) — the diagram now shows the CPU-bound caller path with its O(1) hand-off, the per-class `SendDispatcher` bounded queues/workers, the sender/breaker/throttle stage with the async callback, and the `FallbackDispatcher` with all diversion paths. The component-contract paragraph had already been corrected by the finding-1 fix (`0a7da9b`); with the diagram redrawn, systemic pattern 2 is fully resolved.

### 🟢 Low

- [x] 4. [Build profiles — `./pom.xml:527`, `./pom.xml:538`] {Low} {Confidence: medium} {Dependency & Build Appropriateness} Combined profile activation silently drops a tag group — `-Pexternal-contract,integration` resolves `surefire.excludedGroups` to the later-declared profile's value (`external-contract`), so the explicitly requested external-contract tests are excluded without any signal; the symmetric case drops the integration tests. Rare invocation, surprising outcome. Simpler alternative: an additional combined profile, or excluding via two independent properties so the profiles compose. Reversibility: trivial.
  - **Status:** Fixed in `9ce93df` (2026-08-29) — independent per-stage exclusion properties; each profile blanks only its own part, and a static `never-tagged` sentinel keeps the list non-empty in the all-blanked case (JUnit rejects an entirely empty tag list — discovered by testing exactly that case). All four invocation modes verified green: default 253, `-Pexternal-contract` 257, `-Pintegration -Dgroups=integration` 1, combined 258.
