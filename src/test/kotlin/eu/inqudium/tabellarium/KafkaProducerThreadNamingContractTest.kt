 ...
 * ([SelfLoggingGuard.isOwnProducerThread]) to the Kafka client version
...
class KafkaProducerThreadNamingContractTest {
    ...
    fun `should name the producer network thread prefix plus separator plus client id`() {
        ...
        try {
            ...
            // Then
            assertThat(matching).containsExactly(SelfLoggingGuard.PRODUCER_NETWORK_THREAD_PREFIX + clientId)
        } finally {
